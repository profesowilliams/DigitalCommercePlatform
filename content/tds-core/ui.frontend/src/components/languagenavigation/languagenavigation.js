...
(function () {
  const countriesListModal = document.getElementById('countriesListModal');
  const isExtraReloadDisabled = () => document.body.hasAttribute('data-disable-extra-reload');
  let userIsLoggedIn = !!(!isExtraReloadDisabled() && localStorage.getItem('sessionId'));
  ...
  function hideCountriesModal() {
    ...
    for (let i = 0; i < countryModals.length; i++) {
  ...
  function onDocumentReady() {
    ...
    const languageSelectorOpeners = document.querySelectorAll('.app-open-language-selector');

    for (let i = 0; i < languageSelectorOpeners.length; i++) {
      languageSelectorOpeners[i].addEventListener('click', (event) => {
  ...
  function hideRegionDropdown() {
    const dropdowns = document.getElementsByClassName('cmp-dropdown-content');
    let i;
  ...
  window.addEventListener('click', (event) => {
    if (event.target.matches('.app-open-language-selector') || event.target.closest('.app-open-language-selector')) {
      return;
    }
    if (event.target.matches('.cmp-button__region-select')) {
      ...
      hideCountriesModal();
    } else if (!event.target.matches('.cmp-dropbtn') && !event.target.matches('.cmp-button__region-select')) {
      hideCountriesModal();
      hideRegionDropdown();
    }
    /**
     ...
     */
    const hasDisplayLocationEnabled = isInsideLinkList && isInsideLinkList.hasAttribute('data-displaylocation') === true && isInsideLinkList.dataset.displaylocation === 'true';
