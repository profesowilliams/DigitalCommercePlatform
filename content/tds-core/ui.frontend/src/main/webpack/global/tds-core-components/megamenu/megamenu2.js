...
(function(bp) {
  ...
  function Navigation({navigation}){
    // var that = this;
    var MenuPrimary = '.cmp-megamenu2__primary';
    var MenuSecondary = '.cmp-megamenu2__secondary';
    var MenuTertiary = '.cmp-megamenu2__tertiary';
    var MenuQuaternary = '.cmp-megamenu2__quaternary';
    var itemToSelect = 'cmp-megamenu2__wrapper-item';
    var ActiveWrappClass = 'cmp-megamenu2__wrapper-item--active';
    var ActiveItemClass = 'cmp-megamenu2__item--active';
    var ActiveTabClass = 'cmp-megamenu2__tab--active';
    var ActiveTabPanelClass = 'cmp-megamenu2__tabpanel--active';
    var hasSubMenus = 'cmp-has-submenu';
    var selected = '.cmp-megamenu2__tabpanel.cmp-megamenu2__tabpanel--active ';
    ...
    function init(){
      ...
      var navSecondaryItems = navigation.querySelectorAll(`${MenuSecondary} > ul > li > a`);
      ...
      var navitem = navigation.querySelectorAll('div.cmp-megamenu2__tabpanel');
      ...
      var navTertiaryItems = navigation.querySelectorAll(MenuTertiary + '> ul > li > a');
      ...
      var titlesCloseTertiary = navigation.querySelectorAll(MenuQuaternary + ' .cmp-megamenu2-level-title');
    ...
    function selectPrimaryItem(event){
      ...
      var navSecondaryItems = navigation.querySelectorAll('ol > li > .cmp-megamenu2__tabpanel');
    ...
    function preselectFirstSecondLevelItem(parentMenuItem) {
      var selectedSecondLevelElement = parentMenuItem.querySelector(selected + MenuSecondary + '> ul > li > a.' + ActiveItemClass);

      if(!selectedSecondLevelElement) {
        var firstSubmenuElement = parentMenuItem.querySelector(selected + MenuSecondary + ' > ul > li > a');
    ...
    function navMouseLeave(event){
      ...
      clearItems(MenuPrimary, 'ul > li > a.' + ActiveTabClass, ActiveTabClass);
    }
    ...
    function selectTertiaryItem( event ){
      var target = event.target;
      if (//target.dataset.cmpClickable !== 'true' || target.className.indexOf(ActiveItemClass) >= 0
        target.parentElement.className.indexOf(hasSubMenus) < 0) return; // avoid everything if there is no menu attached to this item
      event.preventDefault();
      MouseLeaveFlag = true;
      clearItems(selected + MenuSecondary, ' > ul > li > a.' + ActiveItemClass, ActiveItemClass);
      target.classList.add(ActiveItemClass);
      var navTertiaryItems = navigation.querySelectorAll(`.${ActiveTabPanelClass} ${MenuTertiary} > ul > li > a`);
  ...
  function preInit() {
    const navigation = document.querySelector('#megamenu2');
    if (!navigation) return;

    const MenuPrimary = '.cmp-megamenu2__primary';
    const ActiveTabPanelClass = 'cmp-megamenu2__tabpanel--active';
    const ActiveTabClass = 'cmp-megamenu2__tab--active';

    document.addEventListener("mouseover", function(e) {
      const megaMenu = e.target.closest('.cmp-megamenu2');
      if (!megaMenu) {
        clearItems(`${MenuPrimary} li a.${ActiveTabClass}`, ActiveTabClass);
        clearItems('.cmp-megamenu2__tabpanel', ActiveTabPanelClass);
      }
    })
  ...
  function onDocumentReady(){
    preInit()
    var navigators = document.querySelectorAll('#megamenu2');
  ...
  function prefixAEMAuthUrlForAEMLinks() {
    ...
    function changeShopLoginUrlPrefix () {
        ...
        if(window.SHOP == undefined) { // ignore if its shop
            let sessionId = localStorage.getItem('sessionId');
            var shopHeaderContainer = document.getElementById("megamenu2");
