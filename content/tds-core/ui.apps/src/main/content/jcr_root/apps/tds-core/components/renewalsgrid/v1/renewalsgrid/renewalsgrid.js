...
use(["../../../common/utils.js"], function(utils) {
    ...
    if (properties) {
        ...
        optionDataList.map((optionDataItem) =>
        ...
        productGridList.map((productGridItem) =>
        ...
        const orderingFromDashboard = utils.fillFieldsDialogProperties(orderingPropertiesList);
        if (orderingFromDashboard != null) {
            orderingFromDashboard.termsAndConditionsLink =
                utils.addHtmlIfNeeded(utils.transformUrlGivenEnvironment(orderingFromDashboard.termsAndConditionsLink));
        ...
        newPurchaseFlyoutList.map((property) =>
            (newPurchaseFlyout[property] = properties[property])
        );
