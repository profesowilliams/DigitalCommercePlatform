...
const MyConfigurations = ({componentProp}) => {
  const {label, endpoint, orderLabels} = JSON.parse(componentProp);
  ...
  const getColors = ( i ) => {
    const elem = document.querySelector(`.cmp-myconfigurations .cmp-dcp-widget-chart__sub-title-color-${i}`);
  ...
  useEffect(() => {
    if(summary){
      ...
      createChart(domRef3.current.getContext('2d'),3,'oldConfigurations','Inactive');
    }
  },[summary])
  useEffect(async () => {
    try{
    ...
    <div className='cmp-dcp-widget cmp-myconfigurations'>
      {summary ? (
          <div className='cmp-dcp-widget-chart__container'>
            <h5 className='cmp-dcp-widget-chart__title'>{label}</h5>
