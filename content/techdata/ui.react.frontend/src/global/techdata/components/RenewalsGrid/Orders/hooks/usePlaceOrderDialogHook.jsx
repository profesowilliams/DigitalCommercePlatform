...
function usePlaceOrderDialogHook({
  successSubmission,
  failedSubmission,
  noResponseMessage,
  orderEndpoints,
  ...
  function handleToggleToaster({
    transactionNumber,
    isSuccess = false,
    failedReason = '',
    salesContentEmail,
  }) {
    const replaceEmail = (messageStr) =>
      messageStr.replace(/\({email}\)/gim, salesContentEmail);
  ...
  async function processOrder() {
    ...
    pushDataLayer(
      getRowAnalytics(
        analyticsCategory,
        renewalData.orderSource === 'Grid'
          ? ANALYTIC_CONSTANTS.Grid.RowActions.Order
          : renewalData.orderSource === 'Details'
          ? ANALYTIC_CONSTANTS.Detail.Actions.OrderDetail
          : ANALYTIC_CONSTANTS.Grid.RowActions.OrderExpanded,
        renewalData
      )
    );

    const operationResponse = await handleOrderRequesting({
      orderEndpoints,
      renewalData,
      purchaseOrderNumber,
      isDetails,
    ...
    typeof resetGrid === 'function' &&
      operationResponse?.isSuccess &&
      setTimeout(() => resetGrid(), 1600);
  }
  ...
  const renderIconComponent = (isTDSynnex) => {
    return isTDSynnex ? (
      ...
      <AutoRenewIcon size={20} fill="white" className="cmp-progress-icon" />
    );
  };
