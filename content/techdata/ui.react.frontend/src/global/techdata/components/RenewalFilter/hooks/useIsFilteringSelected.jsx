...
export default function useFilteringSelected() {
  ...
  const isDateSelectedOrApplied = () => {
    const isRangeDateSelected =
      customStartDate !== null && customEndDate !== null;
    const hasDateApplied =
      getLocalStorageData(FILTER_LOCAL_STORAGE_KEY)?.dateSelected !== null;
    const hasDateSelected =
      dateSelected === 'custom' ? isRangeDateSelected : !!dateSelected;
  ...
  const hasAnyFilterApplied = () => {
    ...
    return (
      noChildIds.some((filter) => filter.checked || filter.applied) ||
      isDateSelectedOrApplied()
    );
  };
  ...
  const hasDateFilterChangeAvailable = () => {
    ...
    const {
      dateSelected: hasLocalDateApplied,
    ...
    // Check if customStartDate or customEndDate differ from localStartDate or localEndDate
    if (
      customStartDate != null &&
      customEndDate != null &&
      localStartDate != null &&
      localEndDate != null &&
      (customStartDate !== localStartDate || customEndDate !== localEndDate)
    ) {
      return true;
    }
    ...
    const hasValidCustomRange =
      hasCustomSelected && !!(customStartDate && customEndDate);
  ...
  const hasFilterChangeAvailable = () => {
     ...
     * @type {boolean}
     */
    const hasFilterListChange = noChildIds.some(
      (filter) =>
        (filter.checked && !filter.applied) ||
        (!filter.checked && filter.applied)
    );
    ...
    if (
      hasFilterListChange &&
      (dateFilterChangeAvailable === null ||
        dateFilterChangeAvailable === undefined)
    ) {
      return true;
    }
