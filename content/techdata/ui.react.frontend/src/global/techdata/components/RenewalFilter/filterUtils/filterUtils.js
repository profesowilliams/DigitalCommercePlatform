...
export function generateFilterFields(filterList, dateSelected, datePickerState) {
    ...
    const uniqueFieldList = [
        ...new Set(filteredResult.map((item) => item.field))
    ];
    const POSTDATA = uniqueFieldList.reduce(
        (ac, field) => ({ ...ac, [field]: mapMultipleValues(field, filteredResult) }),
        {}
    );
