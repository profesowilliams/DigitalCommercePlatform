        ...
        public class Handler : HandlerBase<CustomerFind>, IRequestHandler<Request, Response>
        {
            ...
            public async Task<Response> Handle(Request request, CancellationToken cancellationToken)
            {
                try
                {
                    ...
                    var uri = url.ToUri();
                    var paginatedResponse = await GetAsync<PaginatedResponse<IEnumerable<CustomerModel>>>(uri, request.AccessToken, cancellationToken);
                    return new Response(paginatedResponse.ReturnObject);
                }
                catch (Exception ex)
                {
