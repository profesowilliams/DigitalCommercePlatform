        ...
        public void OnException(ExceptionContext context)
        {
            ...
            else if (context.Exception is UIServiceException uiServiceException)
            {
                ...
                context.Result = new ObjectResult(new ResponseBase<object>
                {
                    Error = new ErrorInformation { 
                        IsError = true,
                        Messages = new List<string> { "UserId : " + uiContext.User.ID + " for TraceId : " + uiContext.TraceId + " " + uiServiceException.Message }, 
            ...
            else
            {
                ...
                context.Result = new ObjectResult(new ResponseBase<object>
                {
                    ...
                    { 
                        IsError = true,
                        Messages = new List<string> { "UserId : " + uiContext.User.ID + " for TraceId : " + uiContext.TraceId + " Something went wrong" },
                        Code = 500
                    }
                });
                context.HttpContext.Response.StatusCode = (int)HttpStatusCode.OK;
