...
namespace DigitalCommercePlatform.UIServices.Order.Infrastructure
{
    public class DigitalFoundationClient : IDigitalFoundationClient
    {
        ...
        public const string Scope = "df.orders";
        public const string ClientId = "UI.Nuance.ServiceToken.ClientId";
        public const string ClientSecret = "UI.Nuance.ServiceToken.ClientSecret";
        ...
        public async Task<TResp> GetAsync<TResp>(string endpoint, string userId)
        {
            ...
            return await GetAsync<TResp>(endpoint, impersonationHeader);
        }

        private async Task<TResp> GetAsync<TResp>(string endpoint, IDictionary<string, string> customHeaders = null)
        {
            _logger.LogDebug($"Getting data for: {endpoint}");

            var headers = _tokenManagerService.GenerateAndAddAuthToken(Scope, ClientId, ClientSecret);
            ...
            try
            {
                var response = await _simpleHttpClient.GetAsync<TResp>(endpoint, null, null, headers);
                return response;
            }
            catch (Exception ex)
            {
