...
namespace DigitalCommercePlatform.UIServices.Commerce.Services
{
    ...
    public class HelperService : IHelperService
    {
        ...
        public async Task<byte[]> GetQuoteDetailsAsXls(QuoteDetails quoteDetails)
        {
            ...
            xlCol = 2;
            SetHeaderImage(xlRow, xlCol, wsQuoteDetail);
        ...
        private void SetHeaderImage(int xlRow, int xlCol, ExcelWorksheet wsQuoteDetail)
        {
            using (var imgRange = wsQuoteDetail.Cells[xlRow, xlCol + 9, xlRow, xlCol + 14])
            {
                imgRange.Merge = true;
                Image img = null;
                var logoUrl = GetResellerLogo();
                if (!string.IsNullOrWhiteSpace(logoUrl))
                    img = GetImageFromUrl(logoUrl);
                if (img == null)
                {
        ...
        private Image GetImageFromUrl(string logoUrl)
        {
            if (string.IsNullOrWhiteSpace(logoUrl))
                return null;
            return null; //temporary & to cheat code analyzer.
        }
        ...
        private string GetResellerLogo()
        {
            return "";
        }
