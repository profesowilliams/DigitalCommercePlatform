...
namespace DigitalCommercePlatform.UIServices.Commerce.Infrastructure.Mappings
{
    ...
    public class QuoteProfile : Profile
    {
        public QuoteProfile()
        {
            ...
            .ForPath(dest => dest.ShipTo.Email, opt => opt.MapFrom(src => src.ShipTo.Contact.FirstOrDefault().Email))
            .ForPath(dest => dest.ShipTo.PhoneNumber, opt => opt.MapFrom(src => src.ShipTo.Contact.FirstOrDefault().Phone))
            .ForPath(dest => dest.ShipTo.Name, opt => opt.MapFrom(src => src.ShipTo.Contact.FirstOrDefault().Name))
            .ForMember(dest => dest.EndUser, opt => opt.MapFrom(src => src.EndUser.Address))
            .ForPath(dest => dest.EndUser.Email, opt => opt.MapFrom(src => src.EndUser.Contact.FirstOrDefault().Email))
            .ForPath(dest => dest.EndUser.PhoneNumber, opt => opt.MapFrom(src => src.EndUser.Contact.FirstOrDefault().Phone))
            .ForPath(dest => dest.EndUser.Name, opt => opt.MapFrom(src => src.EndUser.Contact.FirstOrDefault().Name))
            .ForMember(dest => dest.Reseller, opt => opt.MapFrom(src => src.Reseller.Address))
            .ForPath(dest => dest.Reseller.Email, opt => opt.MapFrom(src => src.Reseller.Contact.FirstOrDefault().Email))
            .ForPath(dest => dest.Reseller.PhoneNumber, opt => opt.MapFrom(src => src.Reseller.Contact.FirstOrDefault().Phone))
            .ForPath(dest => dest.Reseller.Name, opt => opt.MapFrom(src => src.Reseller.Name))
            ...
            .ForMember(dest => dest.Source, opt => opt.MapFrom(src => src.VendorReference.FirstOrDefault().Type + ": " + src.VendorReference.FirstOrDefault().Value))
